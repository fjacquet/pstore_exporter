...
func TestExpandFallback(t *testing.T) {
	unsetForTest(t, "PSTORE_FALLBACK_TEST_UNSET")
	t.Setenv("PSTORE_FALLBACK_TEST_SET", "real")
	t.Setenv("PSTORE_FALLBACK_TEST_EMPTY", "")
	for _, tc := range []struct{ name, in, want string }{
		{"unset falls back", "${PSTORE_FALLBACK_TEST_UNSET:-false}", "false"},
		{"set wins over default", "${PSTORE_FALLBACK_TEST_SET:-false}", "real"},
		{"exported empty falls back", "${PSTORE_FALLBACK_TEST_EMPTY:-other}", "other"},
		{"empty default allowed", "${PSTORE_FALLBACK_TEST_UNSET:-}", ""},
		{"mixed with literal text", "a${PSTORE_FALLBACK_TEST_UNSET:-b}c", "abc"},
	} {
	...
	if _, err := ExpandEnv("${PSTORE_FALLBACK_TEST_UNSET}"); err == nil {
...
func TestExpandSecretRejectsEmpty(t *testing.T) {
	t.Setenv("PSTORE_EMPTY_SECRET_TEST", "")

	if _, err := ExpandEnv("${PSTORE_EMPTY_SECRET_TEST}"); err != nil {
		t.Fatalf("plain expansion must stay lenient on an exported-empty variable: %v", err)
	}
	if _, err := ExpandEnvSecret("password", "${PSTORE_EMPTY_SECRET_TEST}"); err == nil {
