...
// boolean (insecureSkipVerify: true) or as a ${VAR} environment reference resolved
// at secret-resolution time (insecureSkipVerify: ${PSTORE1_SKIP_CERTIFICATE}).
// Backward compatible with existing native-bool configs.
type EnvBool struct {
...
// Resolve expands any ${VAR} reference (via expand) and parses the result to a
// bool. No-op when the value was a native boolean or omitted. Empty expansion
// yields false; a non-boolean expansion is an error.
...
type ArrayConfig struct {
	...
	// InsecureSkipVerify disables TLS certificate verification for this array.
	// Accepts a native YAML boolean or a ${VAR} environment reference (e.g.
	// ${PSTORE1_SKIP_CERTIFICATE}), resolved at secret-resolution time.
