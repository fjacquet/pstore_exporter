...
- `${VAR:-default}` fallbacks in config env references, ported from `pscale_exporter`.
  ...
  nothing is now rejected, so a stray `PSTORE1_PASSWORD=` line fails at startup instead
  ...
  `insecureSkipVerify: "${PSTORE1_SKIP_CERTIFICATE:-true}"`, so the setting is env-driven out of the box
